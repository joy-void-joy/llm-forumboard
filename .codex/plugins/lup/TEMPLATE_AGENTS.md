<!-- Generated from forumboard.devtools.harness.content.template_codex by `uv run lup-devtools harness generate all` — edit the source, not this file. See docs/harness.md. -->
...
`lup-devtools usage claude` the other's; the accounts a launch selects are
curated with `lup-devtools harness profile` — which is not what
`forumboard setup profiles` configures, that being enrolment, the separate
decision about whose claude.ai conversations are read.
