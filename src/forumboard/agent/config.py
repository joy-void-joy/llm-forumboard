...
``.env`` carries the defaults this repository ships; ``.env.local`` carries
the secrets and overrides them, and is gitignored. Everything else imports
``settings`` rather than reaching for ``os.environ``, so what the pipeline is
configured with is answerable by reading one file.

Usage:
    from forumboard.agent.config import settings
    print(settings.sync_interval_seconds)
"""
...
class Settings(BaseSettings, env_file=(".env", ".env.local"), extra="ignore"):
    ...
    notion_token: str | None = Field(
        default=None,
        validation_alias="NOTION_TOKEN",
        description=(
            "Notion internal-integration secret (ntn_… or secret_…). Tools "
            "tagged requires:notion are withheld when this is unset, so the "
            "worldview agent never calls one that could only fail."
        ),
    )
    ...
    notion_parent_page_id: str | None = Field(
        ...
        description=(
            "Page the setup wizard creates the two databases under. Only the "
            "wizard reads it; once the database ids are recorded nothing needs "
            "the parent again."
        ),
    )

    notion_discussions_database_id: str | None = Field(
        default=None,
        validation_alias="NOTION_DISCUSSIONS_DATABASE_ID",
        description="Database holding published discussions and briefings",
    )

    notion_worldview_database_id: str | None = Field(
        default=None,
        validation_alias="NOTION_WORLDVIEW_DATABASE_ID",
        description="Database holding the rewritten current-worldview topic pages",
    )
    ...
    sync_interval_seconds: int = Field(
        default=900,
        validation_alias="FORUMBOARD_SYNC_INTERVAL_SECONDS",
        description="Seconds between conversation-fetch passes over the roster",
    )

    sync_lookback_days: int = Field(
        default=7,
        validation_alias="FORUMBOARD_SYNC_LOOKBACK_DAYS",
        description=(
            "How far back a profile with no recorded cursor reaches on its "
            "first pass, so enrolling someone does not import their whole history"
        ),
    )

    sync_max_conversations: int = Field(
        default=20,
        validation_alias="FORUMBOARD_SYNC_MAX_CONVERSATIONS",
    ...
    worldview_interval_seconds: int = Field(
        ...
        description=(
            "Seconds between worldview passes. Longer than the sync interval "
            "on purpose: a rewrite reads every unmerged discussion at once, so "
            "batching them costs less than rewriting a topic per arrival."
        ),
    )

    briefing_cadences: Annotated[list[str], NoDecode] = Field(
        default=["weekly", "monthly"],
        validation_alias="FORUMBOARD_BRIEFING_CADENCES",
        description=(
            "Briefing windows to write into the discussion database, "
            "comma-separated: daily, weekly, monthly, or quarterly."
        ),
    )

    @field_validator("briefing_cadences", mode="before")
    @classmethod
    def split_briefing_cadences(cls, value: str | list[str]) -> list[str]:
        """Env values are comma-separated cadence names."""
        if isinstance(value, str):
            parts = value.split(",")  # lup: ignore[string-split] — env list format
    ...
    web_host: str = Field(
        default="127.0.0.1",
        validation_alias="FORUMBOARD_WEB_HOST",
        description=(
            "Interface the enrolment server binds. Loopback by default: the "
            "login page drives a browser holding somebody's live claude.ai "
            "session, so reaching it from elsewhere is a deliberate act."
        ),
    )

    web_port: int = Field(
        default=8781,
        validation_alias="FORUMBOARD_WEB_PORT",
