...
RUNNER_TARGETS: list[RunnerTargetRule] = runner_target_rules(
    session_opening=("lup-devtools", "forumboard"),
)
"""What `uv run <target>` may reach here, and where each target has to run.

The checkers are the library's group unchanged. `forumboard` joins
`lup-devtools` outside the sandbox for the same reason and one of its own: the
worldview pass opens an agent session, and the enrolment surface drives a real
browser whose profile directory sits outside the tree. A confined session
would fail at both, and fail in the shape that reads as a broken repository
